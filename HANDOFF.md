...
>
> **修補狀態（2026-07-20，全數清空）**：§1.1 併發競爭（悲觀鎖，6 執行緒實測無超賣）、§1.2 報童極端輸入（422 驗證＋CR≤0.999 保險絲）、§2.2 漏洞 A（username 全域唯一，migration `0153d9a7d3b0`）、漏洞 B（vendor/restock 公司歸屬驗證，Create/Update 雙路徑）、漏洞 C（登入限流 5 次/分/IP，`app/rate_limit.py`）、預購帳約脫鉤（`POST /preorders/{id}/unship` 同交易還原）——**本文件所列風險已全部修補**。
